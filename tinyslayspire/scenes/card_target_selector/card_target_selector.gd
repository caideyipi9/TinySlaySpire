...
var current_card: CardUI # 引用卡片
var targeting := false   # 是否在aiming
...
func _get_points() -> Array:
	...
	start.x += (current_card.scale.x / 2)
	var target := get_local_mouse_position()
	var dist := (target - start)
	
	for i in range(ARC_POINTS):
		var t := (1.0 / ARC_POINTS) * i
		var x := start.x + (dist.x / ARC_POINTS) * i
		var y := start.y + ease_out_cubic(t) * dist.y
